...
%             Clusters.Rad(NumClusters,idx)=DefaultRadii(idx);
    ...
    %% Plot
    if Verbose==1
        hold off;scatter(DataIn(:,1),DataIn(:,1));hold on
        scatter(DataIn(CentreIndex,1),DataIn(CentreIndex,1),'r')
        scatter(DataIn(Include,1),DataIn(Include,1),'g');
        scatter(Clusters.Centre(NumClusters,1),Clusters.Centre(NumClusters,1),'*','r')
        title(sprintf('Clustered: %i, Remaining: %i',size(Results,1)-size(DataIn,1), size(DataIn,1)))
        axis([0 1 0 1])
        drawnow
        for zz=1:size(Clusters.Centre,1)
            rectangle('Position',[Clusters.Centre(zz,1)-Clusters.Rad(zz,1), Clusters.Centre(zz,1)-Clusters.Rad(zz,1), 2*Clusters.Rad(zz,1), 2*Clusters.Rad(zz,1)],'Curvature',[1,1])
        end
    end
...
MergeAny=1;
    while MergeAny==1
        if Verbose==1
            ...
            for zz=1:size(Clusters.Centre,1)
                rectangle('Position',[Clusters.Centre(zz,1)-Clusters.Rad(zz,1),...
                    Clusters.Centre(zz,1)-Clusters.Rad(zz,1), 2*Clusters.Rad(zz,1),...
                    2*Clusters.Rad(zz,1)],'Curvature',[1,1])
            end
            hold on
            scatter(Clusters.Centre(:,1),Clusters.Centre(:,1),'*','r')
            drawnow
        end
        ...
        for idx1=1:max(Results(:,end))
            Clusters.Centre(idx1,:)=mean(Results(Results(:,2)==idx1,1:end-1),1);
            for idx2=1:size(Results,2)-1
                value01=pdist2(Results(Results(:,2)==idx1,idx2),Clusters.Centre(idx1,idx2),'Euclidean');
                if max(value01)>0
